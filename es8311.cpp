...
static bool i2c_reg8_read(uint8_t dev_addr, uint8_t reg_addr, uint8_t *data, uint8_t len) {
    ...
    if (error) { printf("ES8311 I2C read error: %u\n", error); return false; }
    Wire.requestFrom(dev_addr, len);
    for (int i = 0; i < len; i++) *data++ = Wire.read();
    return true;
}
...
static bool i2c_reg8_write(uint8_t dev_addr, uint8_t reg_addr, uint8_t *data, uint8_t len) {
    ...
    uint8_t error = Wire.endTransmission(true);
    if (error) { printf("ES8311 I2C write error: %u\n", error); return false; }
    return true;
}
...
static inline esp_err_t es8311_write_reg(es8311_handle_t dev, uint8_t reg_addr, uint8_t data) {
    ...
    return i2c_reg8_write(es->dev_addr, reg_addr, &data, 1) ? ESP_OK : ESP_FAIL;
}

static inline esp_err_t es8311_read_reg(es8311_handle_t dev, uint8_t reg_addr, uint8_t *reg_value) {
    es8311_dev_t *es = (es8311_dev_t *)dev;
    return i2c_reg8_read(es->dev_addr, reg_addr, reg_value, 1) ? ESP_OK : ESP_FAIL;
}
...
esp_err_t es8311_sample_frequency_config(es8311_handle_t dev, int mclk_frequency, int sample_frequency) {
    ...
    regv &= 0x07; regv |= (c->pre_div - 1) << 5; regv |= c->pre_multi << 3;
    ESP_RETURN_ON_ERROR(es8311_write_reg(dev, ES8311_CLK_MANAGER_REG02, regv), TAG, "I2C error");
    ESP_RETURN_ON_ERROR(es8311_write_reg(dev, ES8311_CLK_MANAGER_REG03, (c->fs_mode << 6) | c->adc_osr), TAG, "I2C error");
    ESP_RETURN_ON_ERROR(es8311_write_reg(dev, ES8311_CLK_MANAGER_REG04, c->dac_osr), TAG, "I2C error");
    ESP_RETURN_ON_ERROR(es8311_write_reg(dev, ES8311_CLK_MANAGER_REG05, ((c->adc_div - 1) << 4) | (c->dac_div - 1)), TAG, "I2C error");
...
static esp_err_t es8311_clock_config(es8311_handle_t dev, const es8311_clock_config_t *clk_cfg, es8311_resolution_t res) {
    ...
    if (clk_cfg->mclk_inverted) reg01 |= BIT(6);
    ...
    if (clk_cfg->sclk_inverted) reg06 |= BIT(5); else reg06 &= ~BIT(5);
...
static esp_err_t es8311_fmt_config(es8311_handle_t dev, const es8311_resolution_t res_in, const es8311_resolution_t res_out) {
    uint8_t reg09 = 0, reg0a = 0, reg00;
    ESP_LOGI(TAG, "ES8311 Slave mode, I2S format");
...
esp_err_t es8311_voice_mute(es8311_handle_t dev, bool mute) {
    ...
    if (mute) reg31 |= BIT(6) | BIT(5); else reg31 &= ~(BIT(6) | BIT(5));
